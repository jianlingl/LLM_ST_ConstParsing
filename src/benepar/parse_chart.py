...
class ChartParser(nn.Module, parse_base.BaseParser):
    ...
    def pred_with_scores(raw_data, return_score=True):
        pass

    def _parse_encoded(
        self, examples, encoded, return_compressed=False, return_scores=False, return_predT_scores=True, use_gold_pos=False
    ):
        with torch.no_grad():
            ...
            if return_scores:
                ...
                predT_scores, charts_np = self.decoder.charts_from_pytorch_scores_batched(
                    span_scores, lengths.to(span_scores.device), return_pred_tree_scores=return_predT_scores
                )
        ...
        for i in range(len(encoded)):
            ...
            if return_scores:
                ...
                if tag_scores is None or use_gold_pos:
                ...
                if return_predT_scores:
                    yield {"score": predT_scores[i], "sent": examples[i].words, "tree": self.decoder.tree_from_chart(charts_np[i], leaves=leaves)}
                else:
    ...
    def parse(
        ...
        if subbatch_max_tokens is not None:
            res = subbatching.map(
                ...
                return_predT_scores=return_predT_scores,
                use_gold_pos=use_gold_pos,
            )
        else:
