...
class ChartDecoder:
    ...
    def charts_from_pytorch_scores_batched(self, scores, lengths, return_pred_tree_scores=False):
        ...
        amax = dist.argmax # todo dist.max 是得分
        amax[..., 0] += 1e-9
        padded_charts = amax.argmax(-1)
        padded_charts = padded_charts.detach().cpu().numpy()
        final_charts = [chart[:length, :length] for chart, length in zip(padded_charts, lengths)]
        if return_pred_tree_scores:
            return dist.max.detach().cpu().numpy(), final_charts
        return None, final_charts
